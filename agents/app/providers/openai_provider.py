...
class OpenAIProvider(BaseAIProvider):
    ...
    KEY_NAMES = ["openai", "api_key", "apiKey", "token"]
    ...
    async def _get_client(self, tokens: Dict[str, Any]) -> AsyncOpenAI:
        ...
        async with self._lock:
            ...
            if not api_key:
                raise ValueError("OpenAI API key not found in tokens. Expected one of: " + ", ".join(self.KEY_NAMES))
    ...
    async def generate(
        ...
        try:
            ...
            logger.info("OpenAI generation successful: {} tokens used", tokens_used)
            return content
            
        except Exception as e:
            logger.error("OpenAI generation error: {}", str(e))
            raise
